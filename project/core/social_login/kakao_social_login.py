...
class KakaoSocialLogin():
    ...
    def login(self, user_data_per_field):
        if not Profile.objects.filter(email=user_data_per_field['email'], social=user_data_per_field['social']):
            raise AssertionError('해당 카카오 계정의 사용자는 존재하지 않습니다.')
        return Profile.objects.get(email=user_data_per_field['email'], social=user_data_per_field['social'])
    ...
    def get_user_data(self, request):
        ...
        user_data = self._parse_user_data(user_social_data)
        return user_data
    ...
    def _get_access_token(self, code):
        ...
        try:
            response = requests.post(
                self.access_token_request_url, headers=headers, data=body)
    ...
    def _get_user_social_data(self, access_token):
        ...
        body = {
            'property_keys':
            '["properties.nickname", "kakao_account.email"]'
        }

        response = requests.post(
            self.user_data_request_url, headers=headers, data=body)
